...
class ParseSemester:
    ...
    def set_dataframe(self) -> None:
        ...
        self.dataframe = (
            pandas.read_csv(self.csv_file, dtype=str)
            .loc[:, self.column_names]
            .dropna(how="any", axis=0, subset=None)
            .set_axis(
                [
                    "class_id",
                    "sis_id",
    ...
    def write_professor_to_database(
        self,
        professor_id: int,
        ...
        if self.professor_exists(professor_id):
            self.logger.log_warning(
                f"ALERT:\t\t{professor_full_name} already in database!"
            )
            return
        professor = Professor(
    ...
    def write_semester_to_database(self):
        ...
        if "Fall" in semester_term_name:
            ...
            semester_code = 2
        semester = Semester(
            year=int(semester_term_name[-4:]),
            name=semester_term_name,
            semester=semester_code,
            discriminator=semester_id,
        )
        semester.save()
    ...
    def parse_csv(self):
        ...
        for _, row in self.dataframe.iterrows():
            ...
            self.write_professor_to_database(
                professor_id,
                professor_full_name,
            ...
            self.write_courses_to_database(
                course_slug, course_department, course, course_id
            )

            # Add the section data here to match the DB fields for sections
            self.write_section_to_database(
                section_slug,
                semester_code,
                course_department,
                course_id,
