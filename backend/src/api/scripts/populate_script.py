...
class ParseSemester:
    ...
    def __read_csv(self):
        ...
        for _, row in data.iterrows():
            if len(row["Room"]) <= 1:
                continue
            professor_id = (
                row["Room"][2] if (1 < len(row["Room"]) <= 3) else row["Room"][3]
            )
            ...
            professor_last_name = (
                row["Room"][0]
                if (1 < len(row["Room"]) <= 3)
                else f"{row['Room'][0]} {row['Room'][1]}"
            )
            professor_first_name = (
                row["Room"][1] if (1 < len(row["Room"]) <= 3) else row["Room"][2]
            )
    ...
    def write_sections(self) -> None:
        for _, s_data in self.sections.items():
            section = Section()
            ...
            professor = Professor.professor.get_professor(
                professor=s_data.get("professor")
            ).first()
